...
public class JsonWriter {
    ...
    public void asJSON(PrintWriter pw, DDS dds, Object specialO) throws NoSuchVariableException, IOException, InvalidDimensionException {
        this.pw = pw;
        pw.print("{\""+dds.getClearName()+"\": {\n");
        Enumeration<BaseType> e = (Enumeration<BaseType>)dds.getVariables();
        while (e.hasMoreElements()) {
            write(e.nextElement());
            if(e.hasMoreElements()) pw.print(", \n");
        }
        pw.print("}\n}");
    }
    
    @SuppressWarnings("unchecked")
    private void write(BaseType type) throws NoSuchVariableException, IOException {
        String name = type.getClearName();

        if (type instanceof DSequence) {
            pw.print("\""+name+"\": {\n");
            while (((ServerMethods)type).read(name, null)) {
                Enumeration<BaseType> e = (Enumeration<BaseType>)((DSequence)type).getVariables();
                while (e.hasMoreElements()) {
                    write(e.nextElement());
                    if(e.hasMoreElements()) pw.print(", \n");
                }
            ...
            if (type instanceof DArray) {
                ...
                writeGrid((DGrid) type); // TODO: Copied from asciiWriter, but not tested!
            } else if (type instanceof DSequence) {
                writeSequence((DSequence) type); // TODO: Copied from asciiWriter, but not tested!
            } else if (type instanceof DStructure) {
                writeStructure((DStructure) type); // TODO: Copied from asciiWriter, but not tested!
            } else {
                writePrimitive(type);
            }
        }        
        ...
        if (dim>1) {
            ...
            for (int n=0; n<length; n++) {
                ...
                if (pv instanceof BaseTypePrimitiveVector) {
                    BaseType bt = ((BaseTypePrimitiveVector)pv).getValue(offset++);
                    writePrimitive(bt);
                } else {
    ...
    @SuppressWarnings("unchecked")
    private void writeGrid(DGrid data) throws NoSuchVariableException, IOException {
        pw.print("\""+data.getClearName()+"\": ");
        writeEnumeration(data.getVariables());
    }
    
    @SuppressWarnings("unchecked")
    private void writeSequence(DSequence data) throws NoSuchVariableException, IOException {
        pw.print("\""+data.getClearName()+"\": ");
    ...
    @SuppressWarnings("unchecked")
    private void writeStructure(DStructure data) throws NoSuchVariableException, IOException {
        pw.print("\""+data.getClearName()+"\": ");
        writeEnumeration(data.getVariables());
    }
    
    private void writeEnumeration(Enumeration<BaseType> e) throws NoSuchVariableException, IOException {
        pw.print("[");
        while (e.hasMoreElements()) {
